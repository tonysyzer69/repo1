...
Usage: !haproxy [paramter1] [paramter2]

paramter1: mode
    - enable
    - disable

paramter2: backend host
    - jiraprod
    - lookerprod
    - lookerdev
    - jirastg
...
EXAMPLE
    ...
    - Enable haproxy jira prod
      Slack> !haproxy enable jiraprod

    - Disable haproxy jira prod
      !haproxy disable jiraprod
EOF
...
#
echo -e "$1\n$2\n" > /home/stanley/.haproxy.log

mode=`grep mode /home/stanley/.haproxy.log|awk -F"=" '{print $NF}'`
backend=`grep backend /home/stanley/.haproxy.log|awk -F"=" '{print $NF}'`
